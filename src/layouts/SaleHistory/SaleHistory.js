...
import { GetBilling,UpdateBilling,DeleteBilling } from "layouts/Api"; // Adjust the path if necessary
...
const BillingHistory = () => {
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      await axios.put(`${UpdateBilling}?id=${currentRowData.id}`, currentRowData);
  ...
  const handleDelete = async (id) => {
    try {
      const del=await axios.delete(`${DeleteBilling}?id=${id}`);
